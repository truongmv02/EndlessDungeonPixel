...
namespace MVT.Base.Dungeon.MapDesign
{
    public class RoomMapEditor : EditorWindow
    {
        ...
        {
            roomStyle = new GUIStyle();
            roomStyle.normal.textColor = Color.white;
            roomStyle.normal.background = EditorGUIUtility.Load("node1") as Texture2D;
            ...
            roomSelectedStyle.padding = new RectOffset(roomPadding, roomPadding, roomPadding, roomPadding);
            roomSelectedStyle.border = new RectOffset(roomBorder, roomBorder, roomBorder, roomBorder);
        ...
        private void ClearAllRoomSelected()
        {
            foreach (var room in roomMap.roomList)
            {
                if (room.DisplayInMapRoom && room.isSelected)
                {
